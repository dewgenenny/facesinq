...
def create_ranking_section(title, scores, empty_message="_No scores yet._"):
    ...
    if not scores:
        blocks.append({"type": "section", "text": {"type": "mrkdwn", "text": empty_message}})
    else:
        for idx, (name, percentage, image_url, score, total_attempts, current_streak) in enumerate(
            scores
        ):
            ...
            if image_url:
                section["accessory"] = {"type": "image", "image_url": image_url, "alt_text": name}
...
def get_leaderboard_blocks():
    ...
    # All Time
    blocks.extend(
        create_ranking_section(
            "🏆 All-Time Legends", all_time_scores, "_Need 10+ attempts to qualify._"
        )
    )
