...
def get_home_view(user_id, team_id):
    ...
    # Hero Section
    blocks = [
        {"type": "header", "text": {"type": "plain_text", "text": "FaceSinq", "emoji": True}},
        {
            "type": "section",
            "text": {
                "type": "mrkdwn",
        ...
        },
        {
            ...
            "elements": [
                {
                    "type": "button",
                    "text": {"type": "plain_text", "text": "▶️ Start Quiz", "emoji": True},
                    "style": "primary",
    ...
    if total_attempts == 0:
        ...
        # Stats State
        streak_text = (
            f"{user.current_streak} 🔥"
            if user and user.current_streak > 0
            else f"{user.current_streak}"
        )
        ...
        blocks.extend(
            [
                {"type": "section", "text": {"type": "mrkdwn", "text": "*Your Stats*"}},
                {
                    "type": "section",
                    "fields": [
                        {"type": "mrkdwn", "text": f"Score: *{score}/{total_attempts}*"},
                        {"type": "mrkdwn", "text": f"Accuracy: *{accuracy:.0f}%*"},
                        {"type": "mrkdwn", "text": f"Streak: *{streak_text}*"},
                    ],
                },
                {
                    "type": "context",
    ...
    blocks.append(
        {"type": "section", "text": {"type": "mrkdwn", "text": "*FaceSinq Game Stats* 🎲"}}
    )
    ...
    blocks.append(
        {
            ...
            "fields": [
                ...
                {"type": "mrkdwn", "text": f"🔥 *Streak Master*\n{streak_master_text}"},
                {"type": "mrkdwn", "text": f"🤓 *Most Dedicated*\n{dedicated_text}"},
            ],
        }
    )
    ...
    blocks.append(
        {
            "type": "section",
            "text": {"type": "mrkdwn", "text": "*Top Players* 🏆"},
            "accessory": {
                "type": "button",
    ...
    if top_scores:
        ...
        blocks.append(
            {"type": "context", "elements": [{"type": "mrkdwn", "text": "_No scores yet._"}]}
        )
    ...
    opt_in_option = (
        ...
        if is_opted_in
        else {"text": {"type": "plain_text", "text": "Disabled"}, "value": "false"}
    )

    blocks.append(
        {
            "type": "section",
            "text": {"type": "mrkdwn", "text": "📅 *Daily Random Quiz*"},
            "accessory": {
                "type": "static_select",
    ...
    # Difficulty Select
    diff_option = (
        {"text": {"type": "plain_text", "text": "Easy (Photo → Name)"}, "value": "easy"}
        if difficulty == "easy"
        else {"text": {"type": "plain_text", "text": "Hard (Name → Photo)"}, "value": "hard"}
    )

    blocks.append(
        {
            "type": "section",
            "text": {"type": "mrkdwn", "text": "🧠 *Difficulty Mode*"},
            "accessory": {
                "type": "static_select",
