...
class TestScoring(unittest.TestCase):
    def setUp(self):
        ...
        with Session() as session:
            ...
            colleague = User(
                id=self.correct_user_id, team_id=self.team_id, name_encrypted="Colleague"
            )
    ...
    @patch(
        "game_manager.delete_quiz_session"
    )  # Don't actually delete so we can reuse setup if needed, or mock it
    def test_scoring_flow(self, mock_delete, mock_get_client):
        ...
        with Session() as session:
            ...
            self.assertEqual(
                score.score, 10
            )  # 10 base + 0 bonus? or 10 base + 1*5? Code: min(streak, 10) * 5. Streak is 1. so 5. 10+5=15.
    ...
    def test_leaderboard(self):
        ...
        for block in blocks:
            if (
                block["type"] == "section"
                and "text" in block
                and "LeaderUser" in block["text"]["text"]
            ):
